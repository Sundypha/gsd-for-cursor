...
```bash
PHASE_DESC=$(node ~/.cursor/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section')
PHASE_REQ_IDS=$(node ~/.cursor/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section // empty' | grep -i "Requirements:" | head -1 | sed 's/.*Requirements:\*\*\s*//' | sed 's/[\[\]]//g' | tr ',' '\n' | sed 's/^ *//;s/ *$//' | grep -v '^$' | tr '\n' ',' | sed 's/,$//')
```
...
```
Task(
  prompt="First, read ~/.cursor/agents/gsd-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
**If found:**
1. read validation template from `~/.cursor/get-shit-done/templates/VALIDATION.md`
...
```bash
node ~/.cursor/get-shit-done/bin/gsd-tools.cjs commit-docs "docs(phase-${PHASE}): add validation strategy"
```
...
```
Task(
  prompt="First, read ~/.cursor/agents/gsd-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
